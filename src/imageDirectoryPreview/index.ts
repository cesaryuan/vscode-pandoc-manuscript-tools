 ...
 * only after the Webview requests another batch. The Webview retains image
 * metadata but mounts a bounded card window, so a large image tree never turns
 * into a matching number of DOM image elements or decoded image bitmaps.
 */
...
class DirectoryPreviewSession {
  ...
  constructor(panel: vscode.WebviewPanel, rootUri: vscode.Uri, scriptUri: vscode.Uri, output: vscode.OutputChannel) {
    ...
    this.output = output;
    this.scanner = new IncrementalImageScanner(rootUri, panel.webview, output);
  }
  ...
  private async handleMessage(message: WebviewMessage): Promise<void> {
    ...
    if (message.type === "rescan") {
      this.scanner.dispose();
      this.scanner = new IncrementalImageScanner(this.rootUri, this.panel.webview, this.output);
  ...
  private async openImage(resourceUri: string): Promise<void> {
    ...
    try {
      imageUri = vscode.Uri.parse(resourceUri);
    } catch (_error) {
      return;
    }

    if (!isSupportedImageUri(imageUri) || !isDescendantUri(this.rootUri, imageUri)) {
      return;
    }
...
class IncrementalImageScanner {
   ...
   */
  constructor(rootUri: vscode.Uri, webview: vscode.Webview, output: vscode.OutputChannel) {
    this.rootUri = rootUri;
    ...
    while (
      ...
      while (
        ...
        if (isDirectory(fileType)) {
          ...
          if (!isSymbolicLink(fileType)) {
            this.pendingDirectories.push({
              uri: childUri,
              relativePath: joinRelativePath(work.relativePath, name),
              nextEntryIndex: 0,
            });
          }
          continue;
        ...
        if (isFile(fileType) && !isSymbolicLink(fileType) && isSupportedImageUri(childUri)) {
          images.push({
            name,
