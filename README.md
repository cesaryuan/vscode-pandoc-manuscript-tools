...
- DOCX and Papper HTML preview buttons in the editor title for Markdown files inside a detected manuscript project when `papper` is on PATH or `uv` is available to install it. DOCX requires saving; HTML preview can render the current unsaved buffer.
...
The DOCX build button is shown only when the active saved Markdown file belongs to a workspace folder with a `style.yml` Papper manuscript configuration. Both build commands use `papper` directly when it is available on PATH. If it is missing, the extension reuses an existing uv tool installation or runs `uv tool install papper` once, then invokes the installed executable directly. The DOCX command runs `papper build docx <markdown-file>` from the detected project root and opens the generated file from `output/docx/`. The HTML preview button uses the same Papper-project check, opens a side Webview beside the Markdown editor, runs `papper build html <markdown-file> --output-file output/html/<name>.html` against a temporary mirror of the current editor buffer, and refreshes the panel after a short debounce. Source and preview scrolling are synchronized proportionally.
